...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # whose tools live in hermes-cli's universe (see toolsets.py). `project`
    # is folded in by this GUI-only resolver itself (desktop Project tools).
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    ...
    # `project` is folded in by this GUI-only resolver (desktop Project tools).
    assert server._load_enabled_toolsets() == ["kanban", "memory", "project"]
...
def test_session_resume_uses_parent_lineage_for_display(monkeypatch):
    ...
    monkeypatch.setattr(
        server, "_set_session_context", lambda target, **_kwargs: []
    )
