...
Keep the root file to the rules that materially steer review and implementation.
The full contribution rubric, examples, and expanded footprint guidance live in
`references/agents-reference.md`.

- **Fix real bugs, well.** Reproduce on current `main`, identify the exact
  failure site, and fix the whole bug class — not just the reported symptom.
...
- **Expand at the edges.** New platforms, providers, models, desktop/TUI work,
  and refactors are welcome when they fit existing setup/config UX.
- **Verify premise and design intent before fixing.** Confirm the bug actually
  manifests on current `main`, and read the original intent before treating an
  omission or restriction as a defect.
- **Preserve invariants.** Do not break prompt caching, role alternation, or
  the byte-stability of the system prompt during a conversation.
- **Prefer E2E + invariant tests.** Use real-path validation for integration
  changes, and avoid snapshot/change-detector tests that merely freeze mutable
  data.
- **Do not add non-secret `HERMES_*` env vars.** Behavioral settings belong in
  `config.yaml`; `.env` is for secrets.
- **Reject speculative or special-cased core growth.** No hooks without a real
  consumer, no plugin-specific core patches, and no mitigations that destroy
  the feature they are meant to secure.
- **Human judgment stays human.** Automated close reasons are narrow
  (`implemented_on_main`, `cannot_reproduce`, `incoherent`); taste-based or
  scope-based rejection decisions stay with maintainers.
- **Preserve contributor credit.** When salvaging external work, build on it in
  a way that keeps authorship intact.
...
├── plugins/              # Plugin system (see `references/agents-reference.md`)
...
- adding tools, config keys, skins, plugins, or skills
